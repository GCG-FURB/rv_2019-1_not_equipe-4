...
public class EventHandlerAnimalFalado : MonoBehaviour, ITrackableEventHandler
{
        ...
        foreach (TrackableBehaviour.Status trackedStatus in mTrackingFound)
        {
            if (newStatus == trackedStatus)
            {
                if (OnTrackingFound != null)
                    OnTrackingFound();
                print("beforePlayAudio");
                if (Main.mark1 && Main.mark2)
                {
                    print("animalIndex: " + Main.animalIndex);
                    print("playAudio");
                    Main.animalFalado[Main.animalIndex].Play();
                }
                return;
            }
        }
