...
public class EventHandlerTrad : MonoBehaviour, ITrackableEventHandler
{
    ...
    public void OnTrackableStateChanged(TrackableBehaviour.Status previousStatus, TrackableBehaviour.Status newStatus)
    {
        ...
        foreach (TrackableBehaviour.Status trackedStatus in mTrackingFound)
        {
            if (newStatus == trackedStatus)
            {
                ...
                {
                    Main.trads[Main.animalIndex].SetActive(true);
                }
                return;
            }
        }
        ...
        foreach (TrackableBehaviour.Status trackedStatus in mTrackingLost)
        {
            if (newStatus == trackedStatus)
            {
                ...
                Main.trads[Main.animalIndex].SetActive(false);
                return;
            }
        }
    }
